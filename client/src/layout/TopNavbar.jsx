...
import { Search, LogOut, Bell, User, Settings as SettingsIcon } from 'lucide-react';
...
const TopNavbar = () => {
  ...
  return (
    <nav className="sticky top-0 z-40 bg-[#1a0f0a]/90 backdrop-blur-xl border-b border-white/5 px-4 md:px-6 py-4 flex justify-between items-center w-full">
      ...
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        <div className="relative w-full max-w-[160px] md:max-w-xs lg:max-w-96 group">
          <Search className="absolute left-3 lg:left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-orange-500 transition-colors" size={16} />
          <input 
            type="text" 
            placeholder="Search vault..." 
            className="w-full bg-white/5 border border-white/10 rounded-full py-2 pl-10 pr-4 lg:px-12 text-sm text-white focus:ring-1 focus:ring-orange-600 outline-none transition-all placeholder:text-gray-600"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 md:gap-6 ml-4">
        ...
        <div className="relative" ref={dropdownRef}>
          <button 
            ...
            className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-gradient-to-br from-orange-500 to-orange-700 flex items-center justify-center text-white font-black border border-white/20 shadow-lg shadow-orange-900/20 uppercase text-xs md:text-sm hover:scale-105 transition-transform"
          >
            {initial}
          </button>
